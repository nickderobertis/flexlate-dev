...
from tests.gen_configs import gen_config_with_user_command
...
def test_read_config_with_commands():
    ...
    assert len(run_config.post_init) == 1
    pre_update_command = run_config.post_init[0]
    assert isinstance(pre_update_command, UserCommand)
    assert pre_update_command.run == "touch user_command.txt"
